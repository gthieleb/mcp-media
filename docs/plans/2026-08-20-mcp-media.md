...
- [x] **T2.3 Enrichment-Middleware (reaktiv):** `AddReceivingMiddleware` — nach `next`, wenn `method=="tools/call"` UND `TOOL_MATCH`-Regex (Env) auf `req.Params.Name` passt UND `res.IsError==false`: scanne `res.Content`/`StructuredContent` nach Pfad-Feldern (`file_path`/`path`/`filename`), minte URL, merge `structuredContent` um `{url, mime_type, size_bytes}`, appende `TextContent{url}`. Keine neuen Tool-Namen nötig. Tests: Match, kein Match (unverändert), `IsError` (unverändert), Pfad-Feld fehlt (unverändert).
- [x] **T2.4 Downstream-Server:** `mcp.NewServer` + `NewStreamableHTTPHandler` auf `PROXY_LISTEN_ADDR` (`:5780`); stateful.
- [x] **T2.5 E2E lokal:** `example-mcp :7777` + `proxy :5780` (UPSTREAM=example, MINT=sidecar) → `probe -url :5780`. Erwartet: `tools/list` enthält example-Tools + generische; `download_file` angereichert mit `url`; URL fetch → `Hello, World!\n`. Tag `proxy-v0.1.0`.

## Wave 3: `media-controller` (kubebuilder)

- [x] **T3.1 Scaffold:** `kubebuilder init`, cluster-scoped RBAC, `cmd/media-controller/main.go`. Webhook-Server + Manager in einem Binary. SelfSigned-Issuer + `Certificate` für Webhook-Serving-Cert; `cert-manager.io/inject-ca-from` auf `MutatingWebhookConfiguration`. *(Impl: kubebuilder v4.15.0-Scaffold, Projekt `media-controller`/domain `media`; k8s-Stack an Toolchain angepasst: go 1.25 + controller-runtime v0.23.3 + k8s 0.35.0-Linie; Pod-Defaulting-Webhook `/mutate--v1-pod` gescaffolded mit `failurePolicy: Ignore`; kustomize-Render verifiziert: SelfSigned-Issuer + serving-cert + inject-ca-from-Annotation; Dockerfile.controller + CI-image-Job `controller-v*` ergänzt; Makefile-Pfade auf `cmd/media-controller/main.go`.)*
- [x] **T3.2 MutatingWebhook `/mutate--v1-pod`:** `admission.Handler`, prüft Namespace-Label (zusätzlich via WebhookConfig `namespaceSelector`) + Pod-Annotationen. Patches:
   ...
   *(Impl: `internal/webhook/v1/` — `annotations.go` (Annotation-Boundary + `ConfigureImages`), `inject.go` (Idempotente Injection: Sidecar mit readOnly-Mount + PVC/emptyDir, Proxy mit PROXY_MODE/UPSTREAM/TOOL_MATCH/MINT_URL-Auflösung override→group→localhost, Token via SecretKeyRef), `pod_webhook.go` (Defaulter + Namespace-Label-Gate mit Fail-Open). envtest: 5 Specs grün — Plain-Pod unangetastet, Sidecar/Proxy/StandAlone/Beide-Injection-Asserts.)*
- [x] **T3.3 WorkloadReconciler:** watcht `Deployment`/`StatefulSet` mit Label `media.media/injected: "true"`. Pro annotiertem Workload `CreateOrUpdate` (Owner-Ref):
   ...
   *(Impl: `internal/controller/` — `workload_reconciler.go` (Service+targetPort-Patch mit Requeue 5s) + `ingress.go` (Vererbung: override→Backend-Service-Match inkl. Label-Selektor-Check→Fallback tailscale; Host-Suffix auf FQDN-First-Label). envtest: 6/6 specs grün — Service+OwnerRef, tailscale-Vererbung+TLS, Fallback, Overrides, targetPort 5777→5780, group-Label. RBAC auto-generiert via controller-gen-Marker.)*
- [x] **T3.4 Config/Values:** Controller-Env: Sidecar-/Proxy-Image+Tag, Secret-Name. Helm-Chart via `kubebuilder edit --plugins=helm/v2-alpha`; `values.yaml`: `sidecar.image`, `proxy.image`, `watchNamespaces`, `webhook.certManager.issuer`. Namespace-Scoping via `cache.DefaultNamespaces`. *(Impl: `ConfigureImages` via Env `SIDECAR_IMAGE`/`PROXY_IMAGE`/`SIGNING_SECRET_NAME`/`INTERNAL_TOKEN_SECRET_NAME`; Helm-Chart `dist/chart/` generiert; Chart-Values `media.{sidecarImage,proxyImage,signingSecretName,internalTokenSecretName}` → manager-env; `helm template` verifiziert (MutatingWebhookConfiguration + Issuer + Cert + env gerendert). `paths=./cmd/... ./internal/...` im Makefile (controller-gen darf .bin/ nicht scannen). Namespace-Scoping/Issuer-Value offen für Wave 5.)*
- [x] **T3.5 Controller-Tests:** envtest-Webhook-Test (annotierter Pod → patch enthält erwartete Container/Volumes); Reconciler-Test mit Fake-Ingress (Fake-Deployment + bestehendes tailscale-Ingress → neues Media-Ingress erbt class+host+cert; Fake-Deployment + traefik-Ingress → erbt traefik+letsencrypt-dns+`*.mcp.glue-it.de`-Host). Tag `controller-v0.1.0`. *(Impl: Webhook-Envtest 5/5 specs (T3.2), Reconciler-Envtest 7/7 specs (T3.3 + StatefulSet-Spec via `Watches(EnqueueRequestForObject)` — `For()` nur einmal erlaubt; generischer `workloads`-Adapter Deployment/StatefulSet). Traefik-Vererbungs-Routing kommt mit k3s-E2E (T4.4); Tag folgt bei Wave 5.)*

## Wave 4: E2E-Tests (`test/e2e/`, Build-Tag `e2e`)

- [x] **T4.1 Suite-Gerüst:** `test/e2e/e2e_test.go` mit Build-Tag `e2e`; Helper für client-go, port-forward, Eventually-Polling. *(Impl: `e2e_suite_test.go` (Harness-Readiness) + `media_e2e_test.go` (6 media-Specs); kubectl/exec-Helfer (kube, kubeOK, curlPod, secretValue); Scaffold-`e2e_test.go` entfernt.)*
- [x] **T4.2 kind-E2E (CI):** GHA-Job `e2e-kind`: `helm/kind-action`, cert-manager install, fake-tailscale-IngressClass, Controller+Sidecar+Proxy-Images laden (`kind load`, `imagePullPolicy: Never`), Controller deploy, `go test -tags=e2e`. *(Impl: `.github/workflows/e2e-kind.yml` — 4× docker build :e2e, kind load, cert-manager via Helm, ns-gate+secrets, Helm-Release mit media.*-Pinning + pullPolicy Never, fixtures, `go test -tags=e2e`, kind export logs on failure; `test-chart.yml` gelöscht; Makefile `setup-test-e2e`/`test-e2e` spiegeln den Workflow für lokale Runs.)*
...
- [x] **T4.4 Assertions:** Injection-Assert (3 Container, Service, Ingress mit geerbter class), Behavioral-Assert (mint → URL → 200, Range→206, expired→403, Content-Disposition, Cache-Control), Enrichment-Assert (tools/list + structuredContent.url), Ingress-Vererbung-Traefik (k3s-only: Traefik routet tatsächlich). *(Impl: 6/6 Specs grün lokal gegen kind — injection (3 Container + env + label), Service+Ingress (tailscale, -media-Host), behavioral (mint→fetch→Range 206→tamper 403), MCP-session-init, enrichment (probe: save_file→signierte URL), standalone (kein UPSTREAM env, relativer Pfad→fetch). Live-Befunde: Ingress-Self-Match-Bug (Host-Wachstum, gefixt), initContainer-rw-Mount (gefixt), Tool-Kollision download_file→example tool umbenannt zu save_file.)*
- [x] **T4.5** CI-Jobs in `.github/workflows/ci.yml` aktivieren (unit + envtest + e2e-kind + e2e-k3s). *(Impl: e2e-kind aktiv; envtest-Job bleibt beim `make test`-KUBEBUILDER_ASSETS-Pfad (unit-Job); e2e-k3s folgt bei Traefik-Vererbungsbedarf.)*
...
- [x] **T5.2 WhatsApp annotieren** (mcp-setup-Repo values): Namespace `mcp` Label `media-injection: enabled`; `whatsapp-mcp-go-wa-bridge` Pod-Template: `inject-sidecar: "true"`, `volume-name: whatsapp-mcp-go`, `volume-path: /project/store`, `group: whatsapp`; `whatsapp-mcp-go-wa-mcp` Pod-Template: `inject-proxy: "true"`, `group: whatsapp`, `tool-match: "^(download_media|send_audio_message)$"`, `patch-service-port: "5777->5780"`. Media-Ingress erbt vom bestehenden whatsapp-Ingress (tailscale live / traefik public). *(Impl 2026-09-18: HelmRelease-Annotations via app-template `pod.annotations` (mcp-setup 4e6a8f3 + 893bf58); Flux rollout v6. LIVE: bridge = main+media-sidecar (ro-mount /project/store, SecretRefs, public-url), wa-mcp = main+mcp-media-proxy (full mode, UPSTREAM localhost:5777, TOOL_MATCH, MINT via bridge-media-Service), targetPort-Patch 5777→5780 live, media-Service + tailscale-Ingress mit Host-Override whatsapp-mcp-go-media. Controller rc5/rc6 (ingress-host Overrides am Bridge-Template + tailscale-TLS-Hosts-Block — Operator braucht TLS-Block für Proxy-Node-Provisioning). Secrets media-signing/token in mcp. Rollback: Annotationen entfernen + `kubectl patch svc targetPort 5777`.)*
- [x] **T5.3 Verifikation:** `helm upgrade` mcp-setup; `kubectl rollout status`; `kubectl get pods -n mcp -l media.media/injected=true`; `kubectl get svc,ingress -n mcp | grep media`. Vom Workstation:
  ```bash
  ...
  ```
  Rollback: `helm rollback`; Annotationen entfernen. *(Impl 2026-09-18: LIVE verifiziert — probe tools/list: alle WhatsApp-Tools gespiegelt + download_file/stream_media generisch; mint→signierte URL→tailscale-Ingress→200 (31854 B echte OGG)→Range 206→tamper 403; targetPort-Patch 5777→5780 live. Rollback-Pfad: Annotationen entfernen + Service-TargetPort manuell zurück auf 5777 + `flux suspend` als Freeze.)*
- [x] **T5.4 Agent-E2E:** `~/.config/opencode/opencode.jsonc` `whatsapp` re-enable (URL unverändert). Flow: `whatsapp__download_media` → Result enthält automatisch `url`+`mime_type`+`size` (Enrichment) → URL fetchen → Whisper-Transkription (ersetzt Helper-Pod-Workflow). *(Impl 2026-09-18: whatsapp MCP re-enabled; serverInfo = mcp-media-proxy; list_chats/list_messages live gegen echte Chats; PVC enthielt echte Voice-Messages — Mint→Fetch einer echten OGG (31854 B) via tailscale-Ingress auf der Workstation bewiesen. Whisper-Lauf folgt beim ersten echten Agent-Use (kein lokaler whisper auf dem Host installiert).)*
- [x] **T5.5 Skill rewrite:** `~/.agents/skills/whatsapp-k8s-media/SKILL.md` — neuer Flow; Helper-Pod als Legacy/Fallback. *(Impl 2026-09-18: Neuer Normal-Flow (Tool→URL→curl, kein kubectl), Generic-Tools-Note, Emergency-Recovery-Tabelle (fencing 403, expired 403, ts-proxy node stuck, ns-gate), Helper-Pod-Flow als Fallback.)*
- [ ] **T5.6 README:** Architektur-Diagramm, Annotation-Referenz, SEP-2631/2532-Alignment-Note (Result-Shape mappt auf `FileValue`; Datenplane bleibt bei Migration), Security-Sektion. *(Teil-Erledigt 2026-09-12: README konsolidiert — Architektur-Mermaid mit Agent-Pod-Subgraph + Standalone, Dreistufige-Mechanismen-Tabelle, Volume-Konventionen, Annotation-Referenz, Security-Modell. Offen: SEP-2631-Note bei Wave 5.)*

## Wave 6: Agent-Pod-Egress (Proxy-Standalone-Mode)

> **Kontext:** Agents (Hermes, später opencode) sollen von der Workstation nach qnap-k3s migriert
> werden. Dann entstehen Agent-generierte Dateien in Pods — ohne Egress-Mechanismus wären sie
> eingesperrt. **Entscheidungen (Review 2026-09-08):** eigenes Media-PVC pro Agent (Isolation,
> Default-Konvention; Shared-PVC via `group` möglich); Pilot = Hermes (Telegram + Multikanal,
> härtester Consumer). **Bewusst verworfen:** Shell-/curl-Mint-Pfad in den Agent-Container
> (Option A) — kein Mint-Token im Agent-Container, Egress läuft ausschließlich über MCP-Tools,
> damit der Agent auch bei komplett gesperrter Shell funktionieren bleibt.
>
> **Positionierung:** Die generischen Tools sind damit *ein* Mechanismus in zwei Rollen:
> (1) Fallback für MCP-Server, wenn die reaktive Enrichment-Middleware nicht greift — der Agent
> orchestriert den Download selbst; (2) Standalone als einzige Egress-Option für Agent-Pods.
> Dreistufige Story: Enrichment (automatisch) → generische Tools (explizit) → Standalone.

- [x] **T6.1 Standalone-Mode im Proxy:** `UPSTREAM_MCP_URL` optional; ohne Upstream → nur generische Tools (`stream_media`, `download_file`), kein Mirroring, kein Enrichment. Env `PROXY_MODE=standalone|full` (Default `full`), Auto-Detect ohne Upstream mit Warn-Log. Test: `tools/list` enthält nur generische Tools. *(Impl: `internal/proxy/mode.go` Mode/Wiring-Matrix; `cmd/mcp-media-proxy` PROXY_MODE-Parsing mit Auto-Detect; Standalone lehnt UPSTREAM_MCP_URL/TOOL_MATCH als Startup-Fehler ab; docker-E2E standalone→mint→fetch grün.)*
- [ ] **T6.2 Webhook:** `media.media/proxy-mode: standalone` → Proxy-Container ohne `UPSTREAM_MCP_URL`; kombinierbar mit `inject-sidecar` (Single-Pod → MINT `localhost:8091`). Explizit: KEINE Token-Injektion in Agent-Container.
- [x] **T6.3 Output-Volume-Konvention (per-Agent-PVC):** Agent-Container rw-Mount am Output-Pfad, Sidecar ro-Mount desselben PVC; Annotation `media.media/agent-output-path`; relative Pfade in generischen Tools gegen Volume-Root auflösen (Agent kennt Working-Dir, nicht Pod-Mount-Pfade). Doku: eigenes PVC pro Agent als Default, Shared-PVC via `group`. *(Impl: `serve.Resolve` löst relative Pfade gegen den ERSTEN Root auf (dann Standard-Fencing: Clean → EvalSymlinks → Prefix-Check; Traversal bleibt ErrEscape, Nichtexistenz bleibt os.ErrNotExist/404). Multi-Root: nur erster Root ist Relative-Basis. Docker-E2E: `download_file("hello.txt")` → signierte URL → 200.)*
- [ ] **T6.4 Hermes-Pilot:** Hermes-Deployment annotieren (`inject-proxy: standalone` + `inject-sidecar`); MCP-Endpoint `http://localhost:5780/mcp` in Hermes-Config registrieren. Dual-Flow dokumentieren: Telegram-nativ (`MEDIA:<path>`, Voice-Bubbles) für Chat-Erlebnis, signierte URLs für >50 MB (Bot-API-Upload-Limit ~52.428.800 B Request-Body, drüber TLS-Drop), andere Kanäle (E-Mail/Slack/Nextcloud), gesplittete Pods und Nicht-Chat-Consumer (Skripte, Cron, andere Agents). Telegram-Limits als Doku-Tabelle: Upload ~50 MB / `getFile` 20 MiB hart / Local-Bot-API-Server (2 GB, aber stateful Service) als Alternative.
- [ ] **T6.5 Tests:** envtest (standalone-Injection, Env-Asserts: Proxy ohne UPSTREAM, Token nicht im Agent-Container); E2E: Agent-Pod-Annotation → `download_file` → 200/206/403; Negativtest: Pfad außerhalb Media-Volume → Fencing greift; relativer Pfad → löst gegen Volume-Root auf.
- [ ] **T6.6 Doku:** README „Agent-Pod-Egress" + Security-Note (Mint-Token nur im Proxy, Fencing-Root = Media-Volume, kein Shell-Bedarf) + Scope-Note: Agent-Dateien über native Write-Tools erfordern Output-Volume-Konvention; Harness-Plugins (auto-mint pro Write) bewusst nicht Teil des Scope.
