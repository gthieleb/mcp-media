...
- [ ] **T6.1 Standalone-Mode im Proxy:** `UPSTREAM_MCP_URL` optional; ohne Upstream → nur generische Tools (`stream_media`, `download_file`), kein Mirroring, kein Enrichment. Env `PROXY_MODE=standalone|full` (Default `full`), Auto-Detect ohne Upstream mit Warn-Log. Test: `tools/list` enthält nur generische Tools.
