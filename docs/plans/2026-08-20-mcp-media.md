...
- [ ] **T6.3 Output-Volume-Konvention (per-Agent-PVC):** Agent-Container rw-Mount am Output-Pfad, Sidecar ro-Mount desselben PVC; Annotation `media.media/agent-output-path`; relative Pfade in generischen Tools gegen Volume-Root auflösen (Agent kennt Working-Dir, nicht Pod-Mount-Pfade). Doku: eigenes PVC pro Agent als Default, Shared-PVC via `group`.
